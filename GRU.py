...
# nor_X_train = np.delete(nor_X_train,35,axis=1)
...
model = func.buildModel(nor_X_train.shape)
callback = EarlyStopping(monitor="loss", patience=10, verbose=1, mode="auto")
model.fit(nor_X_train, nor_Y_train, epochs=50, batch_size=160, validation_data=(nor_X_valid, nor_Y_valid), callbacks=[callback])
model.save('GRU_model.h5')
Y_test = model.predict(nor_X_test)
#+++++++++++++++++++++++++++GRU=============================================================
# pred = pd.DataFrame(pred)
# a = Lscaler.inverse_transform(pred)
...
outdated_items = sales_by_item_id[sales_by_item_id.loc[:,'21':].sum(axis=1)==0]#連續3個月都沒有賣出商品
...
# test.loc[(test['item_cnt_month'] >=0)&(test['item_cnt_month']<0.4) ,'item_cnt_month'] = 0
